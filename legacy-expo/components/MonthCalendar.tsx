...
import { dayKey, dayKeyToDate } from "../lib/VisitsContext";
...
export default function MonthCalendar({
  ...
  const today = dayKey();
  // Noon today, to flag future cells — logging future days is refused at the
  // data layer, so they shouldn't be selectable here (avoids a dead-end where
  // the day reads "No drinks logged" with no way to add any).
  const todayTime = dayKeyToDate(today).getTime();
    ...
    <View className="rounded-[32px] bg-white/[0.05] p-4">
      ...
      <View className="flex-row flex-wrap">
        {cells.map((day, i) => {
          ...
          const isFuture = dayKeyToDate(key).getTime() > todayTime;
