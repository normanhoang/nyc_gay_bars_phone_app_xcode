...
import ZipNote from "../../components/ZipNote";
...
import { useZipQuery } from "../../lib/useZipQuery";
...
export default function ExploreScreen() {
  ...
  // Defaults to Nearest; the sort silently falls back to A–Z until the device
  // location resolves (the `sort === "nearest" && distances` guard below).
  const [sort, setSort] = useState<SortMode>("nearest");
  const [neighborhood, setNeighborhood] = useState<string>(ALL);
  // Search-box state; typing a recognized NYC ZIP jumps the neighborhood
  // filter through the normal selection path (keyboard dismiss, map framing).
  const { query, setQuery, zipNote, setZipNote, onChangeQuery } = useZipQuery(
    (hood) => selectNeighborhood(hood),
  );
  ...
  // Tap the logo → open the Instagram app on the page, web as fallback.
  const openInstagram = useCallback(async () => {
    const app = "instagram://user?username=thenycgaybars";
    const web = "https://www.instagram.com/thenycgaybars";
    try {
    ...
    <Pressable className="flex-1" onPress={Keyboard.dismiss}>
      <View className="px-4 pb-3" style={{ paddingTop: insets.top - 8 }}>
        <View className="mb-1.5 flex-row items-center justify-between">
          ...
          >
            <Image
              // Bare clickable logo, no button frame. Transparent artwork with
              // ~28% internal safe-zone padding, so it's sized up to read well.
        ...
        <Glass radius={16} bordered className="mb-3 flex-row items-center px-3">
          ...
          <TextInput
            ...
            placeholder="Search bars, neighborhoods, ZIP…"
        ...
        <ZipNote note={zipNote} className="-mt-1 mb-3" />
